#!/usr/bin/env python3
"""
通用数据导出系统 - 支持 Mock 和真实测试数据
将 benchmark 数据转换成论文可用的格式（CSV、LaTeX、图表）
...
class PaperDataGenerator:
    ...
    def load_mock_data(self) -> bool:
        ...
        # 查找所有 benchmark JSON 文件
        benchmark_files = list(Path('.').glob('benchmark_level*.json'))

        if not benchmark_files:
            print("   ❌ 未找到 Mock benchmark 数据")
            print("   请先运行: python run_academic_benchmark.py --quick")
            return False

        print(f"   ✅ 找到 {len(benchmark_files)} 个 benchmark 文件")
            ...
            try:
                with open(file) as f:
                    ...
                    if not content.strip():
                        print(f"   ⚠️  跳过空文件: {file.name}")
                        continue
                    data = json.loads(content)
                    if 'benchmarks' in data:
                        self.mock_data['benchmarks'].extend(data['benchmarks'])
            except json.JSONDecodeError as e:
                print(f"   ⚠️  跳过无效JSON文件: {file.name} ({e})")
                continue

        print(f"   ✅ 加载了 {len(self.mock_data['benchmarks'])} 个测试用例")
        return True
    ...
    def load_real_data(self) -> bool:
        ...
        if not real_file.exists():
            print("   ❌ 未找到真实测试数据: real_test_results.json")
            print("   请先运行: python cli_parallel_test.py")
            return False
    ...
    def load_cli_data(self) -> bool:
        ...
        try:
            ...
            print(f"   ✅ Loaded CLI performance data ({test_count} test cases)")
            return True
        except Exception as e:
            print(f"   ⚠️  Error loading CLI data: {e}")
            return False
...
"""
    ...
    def _generate_mock_charts(self):
        ...
        # 提取数据
        names = [b['name'][:30] for b in benchmarks]  # 限制长度
        ...
        print(f"   ✅ {self.mock_dir / 'charts' / 'performance_chart.pdf'}")
    ...
    def _generate_real_charts(self):
        ...
        print(f"   ✅ {self.real_dir / 'charts' / 'serial_vs_parallel.pdf'}")
    ...
    def _generate_comparison_charts(self):
        ...
        print(f"   ✅ {self.comparison_dir / 'charts' / 'speedup_comparison.pdf'}")
...
"""
        ...
        if has_mock:
            readme += """├── mock/                    # Mock environment data
...
"""
        ...
        if has_real:
            readme += """├── real/                    # Real environment data
│   ├── results.csv
│   ├── table.tex
│   └── charts/
"""
        ...
        if has_mock and has_real:
            readme += """├── comparison/              # Mock vs Real comparison
│   ├── mock_vs_real.csv
...
"""
    ...
    def run(self, real_only=False):
        ...
        if has_mock:
            print("   Mock 数据:")
            print("      - mock/results.csv (Excel 可打开)")
            print("      - mock/table.tex (LaTeX 表格)")
            if HAS_MATPLOTLIB:
                print("      - mock/charts/*.pdf (高质量图表)")
        ...
        print("\n📝 文档:")
        print("   - README.md (数据说明)")
        print("   - key_numbers.md (关键数字快速参考)")

        print("\n💡 下一步:")
        print("   1. 查看 README.md 了解数据")
        print("   2. 打开 CSV 文件查看数据")
        print("   3. 复制 LaTeX 表格到论文")
        print("   4. 插入 PDF 图表到论文")
        print("   5. 使用 key_numbers.md 写作")
...
def main():
    parser = argparse.ArgumentParser(
        description="将 benchmark 数据转换成论文格式",
...
示例:
  ...
  python generate_paper_data.py --real-only
    ...
    parser.add_argument(
        "--real-only",
        action="store_true",
        help="仅处理真实测试数据（忽略 Mock）"
    )

    parser.add_argument(
        "--output",
        default="paper_data",
        help="输出目录（默认: paper_data）"
    )
