...
## ⚠️ 重要声明：Mock 测试 vs. 真实环境

**本项目当前所有自动化测试均在 Mock 模式下运行**

### 测试环境说明

| 环境类型 | 用途 | 性能特点 | 代表真实性能 |
|---------|------|---------|------------|
| **Mock 模式** | 算法验证、CI/CD | 理想化性能 | ❌ 否 |
| **真实 API** | 实际部署 | 受网络/限流影响 | ✅ 是 |

### 性能数据标注

本文档中的所有性能数据都会明确标注测试环境：
- 📝 **Mock**: 理论性能，验证算法效率
- 🌍 **真实**: 实际 API 调用，代表真实性能
- 📊 **预期**: 基于 API 特性的真实环境估算

**详细说明**: 请参考 [docs/REAL_WORLD_BENCHMARK.md](docs/REAL_WORLD_BENCHMARK.md)
